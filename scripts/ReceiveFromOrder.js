...
$(function() {
    ...
    $('#txtDocumentDate').datepicker({ showOn: 'button', buttonImage: '../images/calendar_blue.png', buttonImageOnly: true });
    $('#txtFromDate').datepicker({ showOn: 'button', buttonImage: '../images/calendar.gif', buttonImageOnly: true });
    $('#txtToDate').datepicker({ showOn: 'button', buttonImage: '../images/calendar.gif', buttonImageOnly: true });
    $('#btnSearchVendor').button({ icons: { primary: 'ui-icon-search' }, text: false });
    $('#btnLoadDocument').button({ icons: { primary: 'ui-icon-search' }, text: true });
    $('#btnCancel').button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $('#btnSelectDocument').button({ icons: { primary: 'ui-icon-check' }, text: true });
    $('#btnCancelVendor').button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $('#btnSelectVendor').button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnSMOk").button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnSMCancel").button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $("#btnOkSaveDoc").button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnCSaveDoc").button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $("#btnOkAppDoc").button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnCAppDoc").button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $("#btnOkAppDoc2").button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnCAppDoc2").button({ icons: { primary: 'ui-icon-cancel' }, text: true });
    $("#btnOkCancelDoc").button({ icons: { primary: 'ui-icon-check' }, text: true });
    $("#btnCCancelDoc").button({ icons: { primary: 'ui-icon-cancel' }, text: true });
...
function CheckSessionTimeOut() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=CheckSession',
        cache: false,
        datatype: 'json',
        type: 'GET',
...
function CheckDocumentOnLoad() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=chekDoc',
        cache: false,
        datatype: 'json',
        type: 'GET',
        complete: function(xhr, status) {
            if (status == 'success') {
                var data = eval("(" + xhr.responseText + ")");
                SetStatusButton(data.hdfStatus, data.StaffCanRO, data.StaffCanAprov, data.StaffCanCancel)
                DocmentHeader();
            } else {
                parent.location.href = "../logout.aspx"
            }
...
function DocmentHeader() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadDocHeader',
        cache: false,
...
function LoadSummaryDocument() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadDocSum',
        cache: false,
        context: document.body,
        success: function(data) {
...
function CheckDocumentStatus() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=chekDocStatus',
        cache: false,
        datatype: 'json',
        type: 'GET',
        complete: function(xhr, status) {
            if (status == 'success') {
                var data = eval("(" + xhr.responseText + ")");
                SetStatusButton(data.hdfStatus, data.StaffCanRO, data.StaffCanAprove, data.StaffCanCancel)
            } else {
                parent.location.href = "../logout.aspx"
            }
        }
...
function LoadInventory(selectorName) {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadInv',
        cache: false,
        context: document.body,
...
function LoadMaterialGroup() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadMatGroup',
        cache: false,
        context: document.body,
        success: function(data) {
            ary_data = data.split(";");
...
function LoadMaterialDept(materialGroupID) {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadMatDept',
        cache: false,
        context: document.body,
...
function SearchMaterailsByDept(materialDeptID) {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=searchMatByGroupAndCode',
        cache: false,
        type: 'POST',
        datatype: 'xml',
...
function CreateNewDocument() {
    CheckSessionTimeOut();
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=newDoc',
        cache: false,
        context: document.body,
        success: function(data) {
            //alert(data);
...
function SaveDocument() {
    ...
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=saveDoc',
        cache: false,
        context: document.body,
        data: ({ docDate: docDate, inventoryID: inventoryID, vendorID: vendorID, vendorGroupID: vendorGroupID, vendorShopID: vendorShopID, note: note, invoiceNo: invoiceNo }),
        complete: function(xhr, status) {
            //alert(xhr.responseText);
            if (status == 'success') {
                var data = eval("(" + xhr.responseText + ")");
...
function CancelDocument() {
    CheckSessionTimeOut();
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=cancelDoc',
        cache: false,
        datatype: 'json',
        type: 'GET',
        complete: function(xhr, status) {
...
function CheckApproveDocument() {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=CheckApprove',
        cache: false,
        datatype: 'json',
        type: 'GET',
        complete: function(xhr, status) {
...
function ApproveDocument() {
    ...
    //alert(ary_data[0]);
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=ApproveDoc',
        cache: false,
        datatype: 'json',
        type: 'GET',
...
function openDialogSearchDocument(id) {
    ...
    $("#jGDocument").jqGrid({
        //url: 'No',
        colNames: ['เลขที่เอกสาร', 'วันที่เอกสาร', 'สถานะ', 'คลัง', 'ผู้จัดจำหน่าย', 'Adjust', 'หมายเหตุ', 'DocumentShopID', 'DocumentID'],
        colModel: [
...
function SearchDocumentData() {
    ...
    //alert(dInv);
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=searchdoc',
        cache: false,
        type: 'POST',
        datatype: 'xml',
...
function LoadNewDocumentData() {
    ...
    if (newDocID != '' && newShopID != '' || newDocID != 0 && newShopID != 0) {
        $.ajax({
            url: 'DataXML/ReceiveROXML.aspx?action=loadNewDocment',
            cache: false,
            datatype: 'json',
            type: 'GET',
...
// ปิดหน้าต่าง Dialog การค้นหาเอกสารรับตรง
...
function openDialogSearchVendor(id) {
    ...
    $("#jGVendor").jqGrid({
        //url: 'No',
        colNames: ['รหัสผู้จัดจำหน่าย', 'ชื่อผู้จัดจำหน่าย', 'VendorID', 'VendorGroupID', 'VendorShopID', 'VendorAddress', 'PhoneFax'],
        colModel: [
...
function SearchVendorByCodeAndName(searchCode, searchAction) {
    //alert(searchAction);
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=searchVen',
        cache: false,
        type: 'POST',
        datatype: 'xml',
...
function LoadVendorGroup(selectorName) {
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadVendorGroup',
        cache: false,
        context: document.body,
        data: ({ selectorName: selectorName }),
        success: function(data) {
...
function LoadVendor(vendorGroupID, searchVedorBy) {
    ...
    $.ajax({
        url: 'DataXML/ReceiveROXML.aspx?action=loadVen',
        cache: false,
        context: document.body,
...
function addEditMaterialJqGrid() {
    jQuery("#theGrid").jqGrid({
        url: 'DataXML/ReceiveROXML.aspx?action=loadDocDetail',
        datatype: 'xml',
        ...
        onSelectRow: function(rowid) {
           /* if (rowid != selectedRowID) {
                ...
                jQuery("#theGrid").saveRow(selectedRowID, succesfunc, 'DataXML/ReceiveROXML.aspx?action=addMaterialInDocDetial', { mid: materialData.mid, indexlist: selectedRowID, editdocdetailid: materialData.editdocdetailid, IsDiscount: mslDiscount }, aftersavefunc, errorfunc, afterrestorefunc);
    ...
    $("#theGrid").jqGrid('navGrid', '#pager', { edit: false, add: false, del: false, search: false }).navButtonAdd('#pager', {
        ...
        onClickButton: function() {
            ...
            if (rowCheckedDel != '') {
                deleteMaterial('ลบข้อมูล', 'ต้องการลบข้อมูล ใช่หรือไม่', 'ตกลง', 'ยกเลิก', 'DataXML/ReceiveROXML.aspx?action=delMaterialInDocDetial&docdetailid=' + delParam);
            } else {
...
function addRow() {
    ...
    $("#btnAddRow").click(function() {
        $.post('DataXML/ReceiveROXML.aspx?action=addrow',
...
function getPriceFromDropDownUnit(editdocdetailid, materialId, unitId, amount, discount, vat, msldiscount, mPricePerUnit, actionby) {
    ...
    $.getJSON('DataXML/ReceiveROXML.aspx?action=getPricePerUnit', { editdocdetailid: docDetailID, materialId: materialId, unitId: unitId, materialAmount: amount, materailDiscount: mdiscount, materialVat: vat, isDicount: msldiscount, mPricePerUnit: mpriceper, actionby: actionby }, function(data, status) {
...
function setDataToTheGrid(materialData) {
    $.getJSON('DataXML/ReceiveROXML.aspx?action=getMaterialDataJSON', { editDetailId: -1, materialId: materialData.mid, materialCode: materialData.mcode, materialName: materialData.mname, materialAmount: 0, Price: 0, selectUnit: -1, Discouent: 0, IsDiscount: 0, materialVatType: materialData.vattype, actionby: 1 }, function(data, status) {
        //alert(data.d);
        if (data != null) {
            //alert(data.mtotalprice);
...
function setDataWhenEnterSearchJSON(materialCode) {
    ...
    $.getJSON('DataXML/ReceiveROXML.aspx?action=getMaterialDataJSONByCode', { materialCode: materialCode, editDocDetailId: editDocDetailId }, function(data, status) {
        //alert(data.d);
        if (data != null) {
            //alert(data.mtotalprice);
            $("#theGrid").jqGrid('setRowData', selectedRowID, { mid: data.mid });
