<%@ page language="C#" autoeventwireup="true" inherits="Inventory_DailyStock, App_Web_dailystock_new.aspx.9758fd70" enableEventValidation="false" %>

<%@ Register Src="../UserControl/Footer.ascx" TagName="Footer" TagPrefix="uc1" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>Stock Count Daily</title>
    ...
    <script type="text/javascript">
        ...
        function displayMaterial() {
            ...
            $("#theGrid").jqGrid('setGridParam', {
                url: 'DataXML/DailyStockDataXML.aspx?action=load&StaffID=' + require_var.staffId + '&langId=' + require_var.langId + '&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() + '&allowCount=' + allowCount
            }).trigger('reloadGrid');
        ...
        function countStockHistory(historyData) {
            ...
            $("#theGrid").jqGrid().setGridParam({
                url: 'DataXML/DailyStockDataXML.aspx?action=historyDetail&StaffID=' + require_var.staffId + '&langId=' + require_var.langId + '&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() + '&documentDate=' + historyData.stock_date_hide + '&documentId=' + historyData.documentId + '&documentStatus=' + historyData.documentStatus
            }).trigger('reloadGrid');
        ...
        function saveCountStock() {
            ...
            ajaxWorking();
            $.ajax({
                url: 'DataXML/DailyStockDataXML.aspx?action=save&StaffID=' + require_var.staffId + '&langId=' + require_var.langId,
                dataType: 'json',
        ...
        function approveStock() {
            ...
            $("#dialog_approvedocument").dialog({
                title: '<div style="float: left; font-size: 14px;"><div class="icon-vendors" style="float: left; width: 32px; height: 32px; margin-right: 4px;"></div><div style="float: left;"><div style="height: 4px; margin-top: 4px;"></div><div style="margin: 2px;">ยืนยันการอนุมัติเอกสารนับสต๊อก</div></div></div>',
                ...
                resizable: false,
                buttons: {
                    '<%=lbl_2_11.Text %>': function () {
                        $("#dialog_approvedocument").dialog('close');
                    },
                    '<%=lbl_2_10.Text %>': function () {
                        ajaxWorking();
                        checkPermission(
                            19,
                            $("#txtUserName").val(),
                            ...
                            function (data) {
                                ...
                                if (json.Status) {
                                    ...
                                    if (chkNotCount($("#theGrid"))) {
                                        $.ajax({
                                            url: 'DataXML/DailyStockDataXML.aspx?action=approve&StaffID=' + require_var.staffId + '&langId=' + require_var.langId,
                                            dataType: 'json',
                                                ...
                                                if (data.sess_timeout && data.sess_timeout == 1) {
                                                    ...
                                                    if (data.approve == '1') {
                                                        ...
                                                        alertDialog(function () {
                                                            window.location = "DailyStock_New.aspx?StaffID=" + require_var.staffId + '&StaffRoleID=' + require_var.staffRole +
                                                        ...
                                                        alertDialog();
                                                    } else if (data.isNewSession == true) {
                                                        $("#alert_msg").text('<%=lbl_3_29.Text %>');
                                                        alertDialog();
                                                    } else if (data.approve == '-1') {
                                                        $("#alert_msg").text('<%=lbl_3_32.Text %>');
                                                        alertDialog();
                                                    } else {
                                        ...
                                        $("#alert_msg").text(json.ResultText);
                                        alertDialog();
                                    }
                                },
                                function (xhr, status) {
                    ...
                    open: function () {
                        ...
                        find('button:contains("<%=lbl_2_10.Text %>")').button({
                            icons: {
                                primary: 'icon-btn-ok'
                            }
                        });
                        ...
                        $('.ui-dialog-buttonpane').
                        find('button:contains("<%=lbl_2_11.Text %>")').button({
                            icons: {
            ...
            function getCountDate() {
                //alert('getCountDate');
                $.ajax({
                    url: 'DataXML/DailyStockDataXML.aspx?action=getCountDate&StaffID=' + require_var.staffId + '&langId=' + require_var.langId,
                    dataType: 'json',
                    data: 'invId=' + $('#<%=ddlInv.ClientID %> :selected').val(),
                    type: 'POST',
                    cache: false,
            ...
            function cancelStockCount() {
                $("#dialog_approvedocument").dialog({
                    title: '<div style="float: left; font-size: 14px;"><div class="icon-vendors" style="float: left; width: 32px; height: 32px; margin-right: 4px;"></div><div style="float: left;"><div style="height: 4px; margin-top: 4px;"></div><div style="margin: 2px;">ยืนยันการยกเลิกสารนับสต๊อก</div></div></div>',
                    dialogClass: 'dialogWithDropShadow',
                    width: 350,
                    //height: 180,
                    modal: true,
                            ...
                            checkPermission(
                                ...
                                function (data) {
                                    ...
                                    if (json.Status) {
                                        ...
                                        $.ajax({
                                            url: 'DataXML/DailyStockDataXML.aspx?action=cancelDocument&StaffID=' + require_var.staffId + '&langId=' + require_var.langId + '&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() + '&documentId=' + documentId,
                                        type: 'GET',
                                        dataType: 'json',
                                        cache: false,
                                        success: function (data) {
                                            ...
                                            alertDialog(function () {
                                                window.location = "DailyStock_New.aspx?StaffID=" + require_var.staffId + '&LangID=' + require_var.langId + '&StaffRoleID=' + require_var.staffRole;
                                            });
    ...
    <form id="form1" runat="server">
        ...
        <div id="loading" style="display: none; overflow: hidden;">
            <div style="margin-left: 8px;">
                <img src="../Images/loadingSmall.gif" alt="" style="margin-right: 2px;" /><asp:Label ID="lbl_3_1" runat="server" Text="กำลังทำงาน..." Style="font-size: 14px; color: #333; font-family: Segoe UI, Tahoma, Arial;"></asp:Label>
            </div>
        </div>
        ...
        <div>
            <div class="ui-widget ui-widget-header" style="height: 30px; padding: 4px 10px;">
                <div style="float: left; font-size: 14px;">
                    <div class="icon-daily-stock" style="float: left; width: 32px; height: 32px; margin-right: 4px;">
                    </div>
                    <div style="float: left;">
                        <div style="margin: 2px 0 0 2px;">
                            <asp:Label ID="lbl_1_1" runat="server" Text="นับสต๊อกรายวัน"></asp:Label>
                        </div>
                        <div style="height: 4px; font-size: 11px; font-weight: lighter;">
                            <asp:Label ID="lbl_1_2" runat="server" Text="นับสต๊อกเมื่อสิ้นสุดในแต่ละวัน เพื่อปรับจำนวนสต๊อกให้ถูกต้อง"></asp:Label>
                        </div>
                    </div>
                </div>
                <div style="float: right;">
                    <div style="float: left;">
                        ...
                        <span style="border-right: #ccc 1px solid; height: 12px; padding: 4px 0;">&nbsp;</span>
                         <button id="btnPrintHistory" type="button" style="margin-right: 2px;" class="btn">
                            <asp:Label ID="lbl_2_16" Text="พิมพ์ประวัติ" runat="server" class="btn"></asp:Label></button>
                    ...
                    <div style="float: left; display: none;">
                        <select id="slReport" name="slReport">
                            <option value="0"><%=lbl_3_22.Text %></option>
                            <option value="1"><%=lbl_3_23.Text %></option>
                            <option value="2"><%=lbl_3_24.Text %></option>
                        </select>
                    </div>
                </div>
                ...
                <script type="text/javascript">
                    $("select#slReport").selectmenu({ style: 'dropdown' }).change(function () {
                        ...
                        var url = 'StockCountDiffReport.aspx?inv=' + inv + '&StockCountType=daily';
                        if ($(this + ":selected").val() != "0") {
                            if ($(this + ":selected").val() == "1") {
                                ...
                                url = 'StockCountEachMaterialReport.aspx?inv=' + inv + '&StockCountType=daily';
                ...
                $("#btnPrintHistory").button({
                    ...
                    '&stockType=1&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() +
                    ...
                    $("#btnPrintForm").button({
                        ...
                        print_daily = window.open('Report/crreportstockcountmonthly.aspx?invName=' +
                        $('#<%=ddlInv.ClientID %> :selected').text() +
                        '&stockCountName=<%=lbl_1_1.Text %>' + '&stockCountType=dailystockmaterial' +
                        '&stockdate=' + $('#<%=txtLastCountDate.ClientID %>').val() +
                        '&shopId=' + $('#<%=ddlInv.ClientID %> :selected').val() + '&langId=' + require_var.langId, '', 'location=1,status=1,scrollbars=1');
                    });
                                            ...
                                            source: function (request, response) {
                                                $.ajax({
                                                    url: 'DataXML/MaterialListDataXML.aspx?action=smat&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() +
                                                     '&matCode=' + $("#txtMatCode").val() + '&configCountTable=dailystockmaterial' +
                    ...
                    <script type="text/javascript">
                        ...
                        gridComplete: function () {
                            ...
                            if (ids.length > 0) {
                                ...
                                //alert($("#theGrid").jqGrid('getRowData', 1).documentStatus);
                            ...
                            if (data.weeklyCount == 1) {
                                $("#alert_msg").text('<%=lbl_3_27.Text %>');
                                alertDialog();
                            }
                            if (data.monthlyCount == 1) {
                                $("#alert_msg").text('<%=lbl_3_28.Text %>');
                                alertDialog();
                            }
                            //                            if (data.haveWeeklyDoc == true) {
                            //                                if (confirm('มีการนับสต๊อกรายสัปดาห์แล้ว คุณต้องการนับสต๊อกรายวันอีกหรือไม่?')) {
                            //                                    allowCount = 1;
                            //                                    displayMaterial();
                            //                                }
                            //                            }
                   ...
                   $("#txtCountAmount").keyup(function (e) {
                       ...
                       if (e.keyCode == 13) {
                           if ($(this).val() != "" && !isNaN(parseFloat($(this).val()))) {
                               ...
                               $("#validate_amount").html('<%=lbl_3_31.Text %>').addClass('ui-state-error');
        ...
        <div style="display: none; width: auto; overflow: hidden;" id="auto_count_dialog">
            <p>
                ...
                <span id="auto_count_msg" style="font-size: 1.1em;"><%=lbl_3_33.Text %></span>
            </p>
            <p style="border-top: #ccc 1px solid; padding-top: 4px; text-align: right;">
        ...
        <div id="dialog_approvedocument" style="display: none; overflow: hidden; font-family: Segoe UI, Tahoma, Arial;">
            <table class="paramlist admintable" cellspacing="1" width="100%">
                <tr>
                    <td></td>
                    <td>
                        <input type="text" placeholder="รหัสพนักงาน" class="textbox" id="txtUserName" style="font-size: 16px; height: 30px; width: 100%;">
                    </td>
                </tr>
                <tr>
                    <td></td>
                    <td>
                        <input type="password" placeholder="รหัสผ่าน" class="textbox" id="txtPassword" style="font-size: 16px; height: 30px; width: 100%;">
        ...
        <script type="text/javascript">
            $("#btn_ok_auto_count").button({
                ...
                $('#<%=ddlInv.ClientID %>').attr('disabled', true).addClass('ui-state-disabled');
                $("#theGrid").jqGrid('setGridParam', {
                    url: 'DataXML/DailyStockDataXML.aspx?action=load&StaffID=' + require_var.staffId + '&langId=' + require_var.langId + '&invId=' + $('#<%=ddlInv.ClientID %> :selected').val() + '&allowCount=' + allowCount + '&autoCount=1'
            }).trigger('reloadGrid');
            });
        ...
        <asp:Label ID="lbl_3_25" runat="server" Text="ต้องทำการปิดสิ้นวันก่อนจึงจะสามารถอนุมัติการนับสต๊อกได้" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_26" runat="server" Text="ต้องทำการปิดสิ้นวันก่อนจึงจะสามารถนับสต๊อกได้" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_27" runat="server" Text="วันนี้เป็นวันนับสต๊อกรายสัปดาห์ คุณไม่จำเป็นต้องนับสต๊อกรายวัน เพื่อความถูกต้องให้ทำการนับสต๊อกรายสัปดาห์แทน" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_28" runat="server" Text="วันนี้เป็นวันนับสต๊อกรายเดือน คุณไม่จำเป็นต้องนับสต๊อกรายวัน เพื่อความถูกต้องให้ทำการนับสต๊อกรายเดือนแทน" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_29" runat="server" Text="มีวัตถุดิบที่ยังไม่ได้ทำการนับ คุณต้องใส่จำนวนนับให้ครบทุกตัวก่อน ระบบจึงจะยอมให้อนุมัติ" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_30" runat="server" Text="คำเตือน" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_31" runat="server" Text="ใส่จำนวนไม่ถูกต้อง" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_32" runat="server" Text="ไม่สามารถอนุมัติเอกสารนี้ได้ เนื่องจากมีการสร้างเอกสารนับสต๊อกใหม่กว่าเอกสารนี้" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_33" runat="server" Text="นับสต๊อกอัตโนมัติเป็นการนับเท่ากับจำนวนคงเหลือจากระบบ คุณแน่ใจหรือไม่" Style="display: none;"></asp:Label>
        <asp:Label ID="lbl_3_34" runat="server" Text="กด refresh เมื่อมีการเปลี่ยนแปลงการตั้งค่านับสต๊อก" Style="display:none;"></asp:Label>
